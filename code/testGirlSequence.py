...
for f in range(1, seq.shape[-1]):
    ...
    fig = cv2.rectangle(prev_img, (int(tracked_rect[0]), int(tracked_rect[1])),
                        (int(tracked_rect[2]), int(tracked_rect[3])), (0, 0, 0), 1)
    
    #cv2.imwrite(f"results_girl2/fig_{f}.png", fig)
    print("finished image", f)

np.save("code/girlseqrects.npy", rects_sequence)
